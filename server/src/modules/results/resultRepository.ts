    ...
    JSON_ARRAYAGG(
        JSON_OBJECT(
            ...
            'channels', (
                                        ...
                                        SELECT JSON_ARRAYAGG(
                                            JSON_OBJECT(
                                                ...
                                                'lastCorrectResponse', (
                                                    ...
                                                    INNER JOIN fields 
                                                        ON latest_response.question_id = fields.question_id 
                                                        AND fields.status = true  
                                                    INNER JOIN (
                                                    ...
                                                    AND latest_response.date = (
                                                        ...
                                                        FROM response
                                                        INNER JOIN fields ON response.question_id = fields.question_id
                                                        WHERE fields.status = true  
                                                    )
                                                )
                                            )
                                        )
                ...
                FROM channel_slack
                INNER JOIN channel_slack_manager ON channel_slack_manager.channel_slack_id = channel_slack.id
                WHERE channel_slack_manager.manager_id = ?
            )
        )
    ) AS result
FROM enterprise
INNER JOIN manager ON manager.enterprise_id = enterprise.id
WHERE manager.id = ?
    `,
      [id, id],
    );
